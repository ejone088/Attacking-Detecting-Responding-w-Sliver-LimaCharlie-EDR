...
Installed Sliver for Command & Control (C2) capabilities
...
Found the event generated by the credential dump and created a detection & response rule in LimaCharlie to detect future occurrences of the same type of event where the process of the Windows VM ends in lsass.exe and respond by reporting the event on the Detections page
